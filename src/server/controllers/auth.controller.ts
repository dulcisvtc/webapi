...
import { PostAuthLoginDto, PostAuthLogoutDto } from "../dtos/auth.dtos";
...
export class AuthController {
  ...
  @Post("login")
  async login(@Req() req: Request, @Body() data: PostAuthLoginDto): Promise<PostAuthLoginResponse> {
  ...
  @HttpCode(200)
  @Post("logout")
  async logout(@Body() data: PostAuthLogoutDto): Promise<PostAuthLogoutResponse> {
    return await this.authService.logout(data.access_token);
  }

  @Get("sessionInfo")
  async sessionInfo(@Req() req: Request): Promise<SessionInfoResponse> {
