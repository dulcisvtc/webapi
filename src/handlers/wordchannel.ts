...
const handleMessage = async (message: Message) => {
  ...
  if (message.content.startsWith("!") && admens.includes(message.author.id)) return;
  ...
  if (message.author.id === document.user || clean === document.word || clean.slice(0, 1) !== document.word.slice(-1))
