...
export default async function updateSlots(client: Client, eventId?: number) {
  ...
  for (const document of documents) {
    ...
    if (!infoChannel || !slotsChannel || !infoChannel.isTextBased() || !slotsChannel.isTextBased()) break;
    ...
    const embeds = [
      new EmbedBuilder().setTitle(event.name).setURL(`https://truckersmp.com/events/${event.id}`),
      new EmbedBuilder()
      ...
      new EmbedBuilder().setTitle("Route").setImage(event.map),
      new EmbedBuilder()
        .setTitle("Links")
        .setDescription(
          [
    ...
    const msg = (await infoChannel.messages.fetch({ limit: 10 })).find(
      (msg) => msg.author.id === client.user!.id && msg.embeds.length === 4
    );
    try {
      if (!msg) {
      ...
      const embeds = await Promise.all([
        ...
        ...chunk.locations.map(async (location) => {
          ...
          const description = slots.reduce((acc, [slot, { displayName, taken, vtcId }]) => {
            const name = taken ? (vtcId ? `[${displayName}](https://truckersmp.com/vtc/${vtcId})` : displayName) : "*Available*";

            return acc + `**${slot}** - ${name}\n`;
          }, "");

          const embed = new EmbedBuilder().setTitle(location.name).setImage(location.imageUrl);
